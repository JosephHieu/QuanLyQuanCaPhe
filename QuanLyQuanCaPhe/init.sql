...
CREATE DATABASE CafeManagement CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE CafeManagement;

SET NAMES 'utf8mb4';
SET CHARACTER SET 'utf8mb4';
-- ==========================================================
-- BƯỚC 2: TẠO LẠI CÁC BẢNG VỚI VARCHAR(36) (Đã xóa ký tự rác)
-- ==========================================================
...
CREATE TABLE nhanvien (
    ...
    MaChucVu VARCHAR(36) NOT NULL,
    MaTaiKhoan VARCHAR(36) NOT NULL UNIQUE,
...
-- ==========================================================
-- BƯỚC 3: THÊM DỮ LIỆU MẪU (ĐÃ SỬA LỖI LOGIC)
-- ==========================================================
...
-- 3. Bảng TaiKhoan (!!! QUAN TRỌNG: THAY MÃ HASH '123456' CỦA BẠN VÀO ĐÂY)
...
SET @hashed_pw = '$2a$10$cc4Yh/GldgMSJC4myExZduLhkNykfGNhHZ/IsBQERx89/0Z2hdVau'; -- Thay mã hash '123456' của bạn vào
...
(@b6, 'Có khách', 'Bàn 6'), (@b7, 'Trống', 'Bàn 7'), (@b8, 'Trống', 'Bàn 8'), (@b9, 'Đặt trước', 'Bàn 9'), (@b10, 'Đặt trước', 'Bàn 10'); -- Sửa Bàn 10
...
(@nv1, @cv_ql, @tk_admin, 'Nguyễn Văn An (Admin)', '0909111222', '123 Lê Lợi, Q1'),
...
(@nv4, @cv_pv, @tk_phucvu01, 'Phạm Thị Dung (Staff)', '0909444555', '111 Nguyễn Trãi, Q5'),
...
-- 11. Bảng HoaDon (Đã sửa lại TrangThai)
...
(@hd1, 65000.00, '2025-10-25 08:30:00', 1, NULL),  -- Bàn 1 (Trống) -> Đã TT (1)
(@hd2, 76500.00, '2025-10-25 09:15:00', 1, @km1), -- Bàn 2 (Trống) -> Đã TT (1)
(@hd3, 40000.00, '2025-10-25 10:05:00', 0, NULL),  -- Bàn 3 (Có khách) -> Chưa TT (0)
(@hd4, 110000.00, '2025-10-25 11:20:00', 1, @km3), -- Bàn 4 (Trống) -> Đã TT (1)
(@hd5, 49000.00, '2025-10-25 14:00:00', 1, @km6),  -- Bàn 5 (Trống) -> Đã TT (1)
(@hd6, 30000.00, '2025-10-26 08:00:00', 0, NULL),  -- Bàn 6 (Có khách) -> Chưa TT (0)
(@hd7, 75000.00, '2025-10-26 09:00:00', 1, NULL),  -- Bàn 7 (Trống) -> Đã TT (1)
(@hd8, 50000.00, '2025-10-26 09:30:00', 1, @km9),  -- Bàn 8 (Trống) -> Đã TT (1)
(@hd9, 0.00, '2025-10-26 10:10:00', 0, NULL),    -- Bàn 9 (Đặt trước) -> Chưa TT (0)
(@hd10, 0.00, '2025-10-26 10:15:00', 0, NULL);   -- Bàn 10 (Đặt trước) -> Chưa TT (0)

-- 12. Bảng ChiTietThucDon
...
-- Món cho Hóa đơn ĐÃ THANH TOÁN (TrangThai=1)
INSERT INTO chitiethoadon (MaThucDon, MaHoaDon, SoLuong, GiaTaiThoiDiemBan, ThanhTien) VALUES
(@td1, @hd1, 1, 25000.00, 25000.00), (@td2, @hd1, 1, 30000.00, 30000.00), -- HD1
(@td4, @hd2, 1, 45000.00, 45000.00), (@td6, @hd2, 1, 40000.00, 40000.00), -- HD2
(@td8, @hd4, 2, 50000.00, 100000.00), (@td9, @hd4, 1, 30000.00, 30000.00), -- HD4
(@td3, @hd5, 2, 35000.00, 70000.00), -- HD5
(@td1, @hd7, 3, 25000.00, 75000.00), -- HD7
(@td3, @hd8, 1, 35000.00, 35000.00), (@td10, @hd8, 1, 35000.00, 35000.00); -- HD8
-- Món cho Hóa đơn CHƯA THANH TOÁN (TrangThai=0)
INSERT INTO chitiethoadon (MaThucDon, MaHoaDon, SoLuong, GiaTaiThoiDiemBan, ThanhTien) VALUES
(@td6, @hd3, 1, 40000.00, 40000.00), -- HD3 (Bàn 3)
(@td2, @hd6, 1, 30000.00, 30000.00); -- HD6 (Bàn 6)
-- HD9 và HD10 (Bàn 9, 10) là Đặt trước, chưa có món.

-- 16. Bảng ChiTietDatBan (Đã sửa lại logic liên kết)
INSERT INTO chitietdatban (MaBan, MaNhanVien, MaHoaDon, TenKhachHang, SdtKhachHang, NgayGioDat) VALUES
(@b1, @nv4, @hd1, 'Anh Hùng (Đã TT)', '0987654321', '2025-10-25 08:00:00'), -- Bàn 1 (Trống) -> HD 1 (Đã TT)
(@b2, @nv4, @hd2, 'Chị Mai (Đã TT)', '0912345678', '2025-10-25 09:00:00'), -- Bàn 2 (Trống) -> HD 2 (Đã TT)
(@b3, @nv9, @hd3, 'Anh Tuấn (Có khách)', '0911223344', '2025-10-25 10:00:00'), -- Bàn 3 (Có khách) -> HD 3 (Chưa TT)
(@b4, @nv9, @hd4, 'Chị Lan (Đã TT)', '0944556677', '2025-10-25 11:00:00'), -- Bàn 4 (Trống) -> HD 4 (Đã TT)
(@b5, @nv4, @hd5, 'Anh Phát (Đã TT)', '0933445566', '2025-10-25 13:30:00'), -- Bàn 5 (Trống) -> HD 5 (Đã TT)
(@b6, @nv4, @hd6, 'Chị Quyên (Có khách)', '0977889900', '2025-10-26 07:45:00'), -- Bàn 6 (Có khách) -> HD 6 (Chưa TT)
(@b7, @nv9, @hd7, 'Anh Minh (Đã TT)', '0966554433', '2025-10-26 08:30:00'), -- Bàn 7 (Trống) -> HD 7 (Đã TT)
(@b8, @nv9, @hd8, 'Chị Thảo (Đã TT)', '0912121212', '2025-10-26 09:15:00'), -- Bàn 8 (Trống) -> HD 8 (Đã TT)
(@b9, @nv4, @hd9, 'Anh Khoa (Đặt trước)', '0901xxxxxx', '2025-10-26 10:10:00'), -- Bàn 9 (Đặt trước) -> HD 9 (Chưa TT)
(@b10, @nv4, @hd10, 'Chị Linh (Đặt trước)', '0902xxxxxx', '2025-10-26 10:15:00'); -- Bàn 10 (Đặt trước) -> HD 10 (Chưa TT)